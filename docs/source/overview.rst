...
* an iterator, usually a ``ConfigSet`` (see below) which contains ASE ``Atoms`` objects
...
files, or the ABCD database.  A ``ConfigSet`` used for input may be
initialised with
...
* a query to an `ABCD database <https://github.com/libAtoms/abcd>`_
* a list of ``ConfigSet`` objects that use the same type of storage

Similarly, returned configurations can be held in memory,
file(s), or ABCD, depending on the arguments to the ``OutputSpec``
