...
def prepare_ace_params_and_fit(fitting_configs, ACE_name, ace_fit_params, ref_property_prefix='REF_', skip_if_present=False, 
    ...
    ace_fit_params["ACE_fname_stem"] = ACE_name
...
def fit(fitting_configs, ace_fit_params, skip_if_present=False, run_dir='.', 
    ...
    # TODO make sure this is sound -  what if the stem is full path?
    ace_filename = run_dir / (Path(ace_fit_params["ACE_fname_stem"]).name + '.json')
    ...
    # TODO what if relative path is given?
    fit_cfgs_fname_given = Path(use_params["data"]["fname"])
    fit_cfgs_fname = str(run_dir / f'fitting_database.{ACE_name}.extxyz')
    if fit_cfgs_fname_given is not None and fit_cfgs_fname_given.exists():
        warnings.warn(f"Fitting configs filename given with ace_params ({fit_cfgs_fname_given}), exists; "
                      f"Fitting to configs given to ace.fit(), which are in {fit_cfgs_fname}.")

    fit_cfgs_fname = str(run_dir / f'fitting_database.{ACE_name}.extxyz')
    ase.io.write(fit_cfgs_fname, fitting_configs)
    use_params["data"]["fname"] = fit_cfgs_fnameram

    ace_fit_params_filename = ace_filename.parent / (ace_filename.stem + "_fit_params.json")
    ...
    return _execute_fit(cmd, ace_filename, dry_run)


def _read_size(ace_filaname):
    with open(ace_filename.parent / (ace_filename.name + ".size")) as fin:
        info = json.loads(fin.read())
    return info["lsq_matrix_shape"]

def _check_output_files(ace_filename, formats):
    # TODO: update formats depending of whether we need a yace
    for fmt in formats:
        with open(ace_filename.parent / (ace_filename.name + fmt)) as fin:
            if fmt == '.json':
                _ = json.load(fin)
            elif fmt == '.yace':
...
def _execute_fit(cmd, ace_filename, dry_run):
