...
def prepare_params_and_fit(fitting_configs, ACE_fname_stem, ace_fit_params, ref_property_prefix='REF_', 
                           skip_if_present=False, run_dir='.', 
                           ace_fit_exec=str((Path(wfl.scripts.__file__).parent / 'ace_fit.jl').resolve()),
...
def fit(fitting_configs, ace_fit_params, skip_if_present=False, run_dir='.', 
    ...
    ace_file_base = run_dir / (Path(ace_fit_params["ACE_fname_stem"]).name )
    ace_fit_params["ACE_fname_stem"] = str(ace_file_base)
    ...
    fit_cfgs_fname = str(run_dir / f'fitting_database.{ace_file_base.name}.extxyz')
    ...
    ace_fit_params_filename = ace_file_base.parent / (ace_file_base.name + "_fit_params.json")
...
def _read_size(ace_file_base):
    with open(ace_file_base.parent / (ace_file_base.name + ".size")) as fin:
...
def _check_output_files(ace_file_base, formats):
    ...
    for fmt in formats:
        with open(ace_file_base.parent / (ace_file_base.name + fmt)) as fin:
            if fmt == '.json':
                _ = json.load(fin)
            elif fmt == '.yace':
