...
def prepare_params_and_fit(fitting_configs, ACE_fname_stem, ace_fit_params, ref_property_prefix='REF_', skip_if_present=False, 
                               run_dir='.', ace_fit_exec=str((Path(wfl.scripts.__file__).parent / 'ace_fit.jl').resolve()),
                               dry_run=False, verbose=True, remote_info=None, wait_for_results=True):
    ...
    ACE_fname_stem: str
        name of ACE potential, saved into files with suffix
    ace_fit_params: dict
        dict with all fitting parameters for ACE1pack, 
        to be updated with ACE_fname_stem and ref_property_prefix 
    ...
    """
    # why can't we accept no prop prefix? 
    assert isinstance(ref_property_prefix, str) and len(ref_property_prefix) > 0

    ace_fit_params = deepcopy(ace_fit_params)

    if "ACE_fname_stem" in ace_fit_params.keys():
        warnings.warn(f"Fitting to {ACE_fname_stem}, not {ace_fit_params['ACE_fit_params']} found in ace_fit params")
    ace_fit_params["ACE_fname_stem"] = ACE_fname_stem
    ...
    if len(isolated_atoms) > 0 and "e0" in ace_fit_params.keys():
        raise RuntimeError("Got e0 both as an isolated atom and in ace_fit_params")
    if "e0" not in ace_fit_params.keys():
        ace_fit_params["e0"] = _prepare_e0(isolated_atoms, ref_property_prefix)
...
def fit(fitting_configs, ace_fit_params, skip_if_present=False, run_dir='.', 
    ...
    ace_filename_base = run_dir / (Path(ace_fit_params["ACE_fname_stem"]).name )
    ace_fit_params["ACE_fname_stem"] = str(ace_filename_base)
    ...
    if skip_if_present:
        try:
            if dry_run:
                return _read_size(ace_filename_base)

            _check_output_files(ace_filename_base, [".json"])

            return str(ace_filename_base)
    ...
    fit_cfgs_fname = str(run_dir / f'fitting_database.{ace_filename_base.name}.extxyz')
    ...
    ace_fit_params_filename = ace_filename_base.parent / (ace_filename_base.name + "_fit_params.json")
    print(ace_fit_params_filename)
    with open(ace_fit_params_filename, "w") as f:
        f.write(json.dumps(use_params))

    cmd = f"{ace_fit_exec} --fit-params {ace_fit_params_filename} "
    if dry_run: 
        cmd += "--dry-run "
    cmd +=  f"> {ace_filename_base}.stdout 2> {ace_filename_base}.stderr "

    if verbose:
        print('fitting command:\n', cmd)

    return _execute_fit(cmd, ace_filename_base, dry_run)


def _read_size(ace_filename_base):
    with open(ace_filename_base.parent / (ace_filename_base.name + ".size")) as fin:
        info = json.loads(fin.read())
    return info["lsq_matrix_shape"]

def _check_output_files(ace_filename_base, formats):
    # TODO: update formats depending of whether we need a yace
    for fmt in formats:
        with open(ace_filename_base.parent / (ace_filename_base.name + fmt)) as fin:
            if fmt == '.json':
                _ = json.load(fin)
            elif fmt == '.yace':
...
def _execute_fit(cmd, ace_filename_base, dry_run):
    ...
    try:
        ...
        with open(ace_filename_base.parent / (ace_filename_base.name +'.stdout')) as fin:
            for l in fin:
                print('STDOUT', l, end='')

        with open(ace_filename_base.parent / (ace_filename_base.name +'.stderr')) as fin:
            for l in fin:
                print('STDERR', l, end='')
    ...
    for at in fitting_configs:
        if ref_property_prefix + 'stress' in at.info:
            ...
            if len(stress.shape) == 2:
                ...
                # Voigt 6-vector
                at.info[ref_property_prefix + 'virial'] = np.array((-voigt_6_to_full_3x3_stress(stress) * at.get_volume()).ravel())


def _prepare_e0(isolated_atoms, ref_property_prefix):
    e0 = {}
