...
def test_configset_from_atoms():
    ats = [Atoms('H'), Atoms('C')]
    c_ats = ConfigSet(input_configs=ats)
...
def test_configset_from_files(tmp_path):
    ...
    c_ats = ConfigSet(input_files=[str(tmp_path / 'at1.xyz'), str(tmp_path / 'at2.xyz')])
...
def test_configset_from_ConfigSets(tmp_path):
    ...
    c_ats = ConfigSet(input_configsets=[ConfigSet(input_files=str(tmp_path / 'at1.xyz')), ConfigSet(input_files=str(tmp_path / 'at2.xyz'))])
...
def test_configset_from_ConfigSets_mismatched(tmp_path):
    ...
    with pytest.raises(RuntimeError):
        c_ats = ConfigSet(input_configsets=[ConfigSet(input_files=str(tmp_path / 'at1.xyz')), ConfigSet(input_configs=ats)])


def test_merge():
    ats1 = [Atoms('H'), Atoms('C')]
    c_ats1 = ConfigSet(input_configs=ats1)

    ats2 = [Atoms('B'), Atoms('F')]
    c_ats2 = ConfigSet(input_configs=ats2)
...
def test_scratch(tmp_path):
    ...
    ci = ConfigSet(input_files=[str(tmp_path / 'at1.xyz'), str(tmp_path / 'at2.xyz')])
